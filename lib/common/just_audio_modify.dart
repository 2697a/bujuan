...
class AudioPlayer {
  ...
  AudioPlayer({
    ...
    _processingStateSubject.addStream(playbackEventStream.map((event) => event.processingState).distinct().handleError((Object err, StackTrace stackTrace) {
      /* noop */
    }));
    _bufferedPositionSubject.addStream(playbackEventStream.map((event) => event.bufferedPosition).distinct().handleError((Object err, StackTrace stackTrace) {
      /* noop */
    }));
    _icyMetadataSubject.addStream(playbackEventStream.map((event) => event.icyMetadata).distinct().handleError((Object err, StackTrace stackTrace) {
      /* noop */
    }));
    ...
    _currentIndexSubject.addStream(playbackEventStream.map((event) => event.currentIndex).distinct().handleError((Object err, StackTrace stackTrace) {
      /* noop */
    }));
    _androidAudioSessionIdSubject.addStream(playbackEventStream.map((event) => event.androidAudioSessionId).distinct().handleError((Object err, StackTrace stackTrace) {
      /* noop */
    }));
  ...
  Future<Duration?> setFilePath(
    ...
    bool preload = true,
      ...
      setAudioSource(AudioSource.uri(Uri.file(filePath)), initialPosition: initialPosition, preload: preload);
...
class ConcatenatingAudioSource extends AudioSource {
  ...
  Future<void> clear() async {
    ...
    if (_player != null) {
      _player!._broadcastSequence();
      await (await _player!._platform)
          .concatenatingRemoveRange(ConcatenatingRemoveRangeRequest(id: _id, startIndex: 0, endIndex: children.length, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
...
class ResolvingAudioSource extends StreamAudioSource {
  ...
  bool _hasRequestedSoundUrl = false;
  ...
  Future<StreamAudioResponse> request([int? start, int? end]) async {
    // if (!_hasRequestedSoundUrl || DateTime.now().millisecondsSinceEpoch - _timestamp > 30 * 1000) {
    //   if (_soundUrlCompleter.isCompleted) _soundUrlCompleter.completeError('error');
    //   print('===============需要获取url============');
    //   _hasRequestedSoundUrl = true;
    //   _timestamp = DateTime.now().millisecondsSinceEpoch;
    //   _soundUrl = await resolveSoundUrl(uniqueId);
    // }
    print('===============需要获取url============');
    final soundUrl = await resolveSoundUrl(uniqueId);
    if (soundUrl == null || (soundUrl.path).isEmpty) {
    ...
    final acceptRangesHeader = response.headers.value(HttpHeaders.acceptRangesHeader);
    ...
    return StreamAudioResponse(
        rangeRequestsSupported: acceptRangesHeader != null && acceptRangesHeader != 'none',
        sourceLength: null,
  ...
  @override
  AudioSourceMessage _toMessage() {
    return ProgressiveAudioSourceMessage(id: _id, uri: _uri.toString(), headers: headers, tag: tag);
  }
}
