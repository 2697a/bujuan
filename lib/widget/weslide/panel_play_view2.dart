...
class SlidingUpPlayViewPanel2 extends StatefulWidget {
  ...
  final PlayViewPanel2Controller? controller;
  ...
  @override
  _SlidingUpPlayViewPanel2State createState() => _SlidingUpPlayViewPanel2State();
}

class _SlidingUpPlayViewPanel2State extends State<SlidingUpPlayViewPanel2> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _ac = AnimationController(
        ...
        value: widget.defaultPanelState == PanelState.CLOSED ? 1.0 : 1.0 //set the default panel state (i.e. set initial value of _ac)
        )
      ..addListener(() {
    ...
    scale = Tween(begin: 1.0, end: .8).animate(_ac);
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
        ...
        // Container(),
        SizedBox(width: 100,height:Get.height,child: widget.panel ?? Container(),),
        _gestureHandler(
          child: AnimatedBuilder(
            animation: _ac,
            builder: (context, child) {
              return ScaleTransition(scale: scale,child: Container(
                height: Get.height,
                margin: EdgeInsets.only(left: 100 * (1 - _ac.value),top: 30*(1 - _ac.value),bottom: 30*(1 - _ac.value)),
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    if (widget.panel != null) {
      return GestureDetector(
        onHorizontalDragUpdate: (DragUpdateDetails dets) => _onGestureSlide(dets.delta.dx),
  ...
  void _onGestureSlide(double dy) {
    ...
    if (!_scrollingEnabled && widget.isDraggable) {
      if (widget.slideDirection == SlideDirection.UP) {
        _ac.value -= dy / (widget.maxHeight - widget.minHeight);
        widget.animationController?.value -= dy / (widget.maxHeight - widget.minHeight);
      } else {
        _ac.value += dy / (widget.maxHeight - widget.minHeight);
        widget.animationController?.value += dy / (widget.maxHeight - widget.minHeight);
      }
  ...
  void _onGestureEnd(Velocity v) {
    ...
    double visualVelocity = -v.pixelsPerSecond.dx / (widget.maxHeight - widget.minHeight);
    ...
    if (v.pixelsPerSecond.dx.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        if (v.pixelsPerSecond.dx.abs() >= kSnap * minFlingVelocity || minDistance == d2Snap) {
...
class PlayViewPanel2Controller {
  _SlidingUpPlayViewPanel2State? _panelState;

  void _addState(_SlidingUpPlayViewPanel2State panelState) {
