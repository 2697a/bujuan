...
class LyricPainter extends ChangeNotifier implements CustomPainter {
  ...
  LyricPainter(TextStyle style, this.lyric, {this.textAlign = TextAlign.left, Color highlight = Colors.red}) {
    assert(lyric != null);
    lyricPainters = [];
...
class LyricEntry {
  ...
  LyricEntry(this.line, this.position, this.duration) : this.timeStamp = BuJuanUtil.getTimeStamp(position);
