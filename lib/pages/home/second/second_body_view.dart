...
class SecondBodyView extends GetView<HomeController> {
      ...
      children: [
        Obx(() => Container(
          width: Get.width,
              ...
              child: Text(controller.mediaItem.value.title),
            )),
        Column(
          children: [
            Obx(() => SizedBox(height: controller.getPanelMinSize() + controller.getTopHeight())),
            Obx(() => Padding(
            ...
            return Padding(
              ...
              child: FlutterRangeSlider(
                  ...
                  handler: FlutterSliderHandler(
                      decoration: BoxDecoration(borderRadius: BorderRadius.circular(4), color: controller.rx.value.dark?.color.withOpacity(.7) ?? Colors.black12),
                  ...
                  trackBar: const FlutterSliderTrackBar(activeTrackBarHeight: .1, inactiveTrackBarHeight: .1),
                  onDragging: (a, b, c) => controller.audioServeHandler.seek(Duration(milliseconds: (controller.mediaItem.value.duration?.inMilliseconds ?? 0) * b ~/ 100))),
            );
          }),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: 40.w),
  ...
  Widget _buildPlayController() {
    return Expanded(
        child: Obx(() => Padding(
              padding: EdgeInsets.only(left: 40.w, right: 40.w, bottom: controller.isRoot.value ? controller.secondPanelHeaderSize : 0),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.center,
                children: [
                  IconButton(onPressed: () => controller.changeRepeatMode(), icon: Icon(controller.getRepeatIcon(), color: controller.rx.value.dark?.bodyTextColor)),
                  Expanded(
                      child: Row(
                  ...
                  IconButton(onPressed: () => controller.changeShuffleMode(), icon: Icon(controller.getShuffleIcon(), color: controller.rx.value.dark?.bodyTextColor)),
                ],
              ),
            )));
  }
